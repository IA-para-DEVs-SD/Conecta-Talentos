...
class VagaORM(Base):
    ...
    __table_args__ = (Index("ix_vagas_titulo", "titulo"),)
...
class CurriculoORM(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    vaga_id = Column(
        Integer, ForeignKey("vagas.id", ondelete="CASCADE"), nullable=False
    )
...
class AnaliseORM(Base):
    ...
    curriculo_id = Column(
        Integer,
        ForeignKey("curriculos.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
    )
